...
export function TopCustomers({ sessions }: TopCustomersProps) {
  ...
  const getRankIcon = (index: number) => {
    ...
    return <span className="text-gray-500 font-bold w-4 text-center">#{index + 1}</span>;
  };
  ...
  return (
    <div className="bg-[#1a1a24] rounded-xl border border-gray-800 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-gray-400 text-sm">By lifetime spend</span>
      </div>
      ...
      <div className="space-y-3">
        {topCustomers.map((customer, index) => (
          <div
            key={customer.phone}
            className="flex items-center justify-between p-3 bg-[#0a0a0f] rounded-lg border border-gray-800 hover:border-gray-700 transition-colors"
          >
            <div className="flex items-center gap-3">
              {getRankIcon(index)}
              <div>
                <p className="text-white font-medium">{maskPhone(customer.phone)}</p>
                <p className="text-gray-400 text-xs">{customer.visitCount} visits</p>
              </div>
            </div>
